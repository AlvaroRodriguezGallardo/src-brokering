...
Note that function above does not have explicitly number of cores. It may be studied before. For time, it was considered successions of functions $\{\frac{1}{x^p}\}$. Although $\{x^p\}$ where $p \in \mathbb{N}$ is an option, and fulfills restrictions on energy consumption in CPU/GPU/ARM, or other hardware componentes, I do not know if cores function should have a $x^p$ behaviour.
